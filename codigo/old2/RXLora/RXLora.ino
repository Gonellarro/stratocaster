...
// Uncomment and fill if your broker requires authentication:
...
// Extracts the next field from 'src' up to 'sep', advances src past the separator.
// Returns the extracted field.
String extractField(String& src, char sep) {
  int idx = src.indexOf(sep);
  if (idx < 0) {
    // Last field or malformed — return what's left
    String field = src;
    src = "";
    return field;
  }
...
// Parses packet format: lat,lng;date;time;altitude;course;speed
...
bool parsePacket(const String& raw, GpsData& out) {
  ...
  out.speed        = buf;  // last field — remainder
...
// Non-blocking reconnect: attempts once per MQTT_RETRY_MS
void maintainMqtt() {
...
void setupWebServer() {
  webServer.on("/", HTTP_GET, [](AsyncWebServerRequest* request) {
    String mapsUrl = "https://www.google.com/maps?q=" + lastFix.lat + "," + lastFix.lng;
